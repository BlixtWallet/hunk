# AI Inline Diff Implementation Plan
...
- the inline pane renders a unified up-and-down diff, not the Review tab's side-by-side layout
- the inline pane header includes an `Open in Review` action for the full Review experience
...
The important architectural constraint is that we should reuse the existing review compare loading and syntax/highlight machinery, but we should not reuse the side-by-side Review surface state as-is. The AI pane needs its own renderer and surface state because it is width-constrained and intentionally unified rather than split.

## Product Definition

### V1 behavior

- Clicking an AI diff-summary row opens an inline diff pane instead of switching tabs.
...
- `crates/hunk-desktop/src/app/render/ai_workspace_sections.rs` already has a dormant right-side inline review split and pane chrome.
- `crates/hunk-desktop/src/app/controller/ai/workspace_surface.rs` already has thread-scoped inline-review selection state and an unused `ai_open_inline_review_for_row(...)`.
...
The main missing piece is not data loading. The main missing piece is a dedicated AI-side surface that consumes the same review session data but renders it as a unified stacked diff.
...
- V1 assumes the AI inline diff shows the same compare snapshot the Review tab would show for the selected thread/workspace. That preserves current semantics and avoids inventing turn-specific patch reconstruction in the first pass.
...
That coupling is fine for the Review tab, but it is the wrong shape for the AI pane. The AI pane needs a separate surface state because:
...
- add a separate AI inline diff surface state for scroll, cached inline snapshot, and inline selection

### 3. Add a unified inline-diff viewport model

The AI pane should not render the Review tab's left/right viewport directly. Instead it should build an AI-specific viewport snapshot from the already-projected review display rows.
...
### Phase 1. Wire the existing inline-pane shell to the right interaction

Goal: make AI diff-summary clicks open the existing split-pane shell instead of switching tabs.
...
- Reserve `ai_open_review_tab(...)` for the explicit header button inside the inline pane.
...
Goal: let both the Review tab and AI inline pane consume the same compare session and display rows without sharing scroll/layout state.
...
### Phase 3. Build an AI-specific unified viewport snapshot from review display rows

Goal: transform the existing review display-row output into stacked AI-pane rows without recomputing syntax.
...
- Build the AI inline viewport from:
  - `ReviewWorkspaceSession`
  - shared display rows
  - shared syntax spans
...
- The AI pane should consume the existing display-row/syntax-span output, not raw diff-summary text from the AI timeline row.
...
### Phase 5. Render the AI inline diff pane

Goal: replace the placeholder Review-surface embedding with a dedicated unified AI diff surface.
...
- Use a dedicated scroll handle for the inline diff body.
...
- If the selected diff row disappears from the thread, clear the inline-review selection.

Nice-to-have but not required for V1:

- persist the right-pane width
- persist inline selected file/row per thread
...
- `ai_workspace_render_tests.rs`
  - diff-summary click opens inline review instead of switching tabs
  - non-diff blocks do not trigger inline review
- controller tests under `controller/ai/tests`
  - thread-scoped inline-review selection persists correctly
...
Pulling display-row generation out of `ReviewWorkspaceSurfaceState` touches a central path. Keep the refactor small and mechanical before adding the unified renderer.
...
- Should the AI pane expose file-header `Open file` actions in V1, or should that remain Review-only?
- Do we want to persist inline pane width and selected file per thread in the first pass, or keep V1 stateless beyond open/closed row selection?

## Recommended Order Of Work

1. Wire diff-summary clicks to the existing inline-review selection state.
...
5. Render the new AI inline diff pane and header actions.
