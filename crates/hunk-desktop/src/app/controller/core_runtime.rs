impl DiffViewer {
    ...
    fn next_graph_refresh_epoch(&mut self) -> usize {
        self.graph_refresh_epoch = self.graph_refresh_epoch.saturating_add(1);
        self.graph_refresh_epoch
    }

    fn cancel_pending_graph_refresh(&mut self) {
        self.next_graph_refresh_epoch();
        self.graph_refresh_task = Task::ready(());
    }

    fn schedule_background_graph_refresh(&mut self, cx: &mut Context<Self>) {
        let epoch = self.next_graph_refresh_epoch();
        self.graph_refresh_task = cx.spawn(async move |weak_this, cx| {
            cx.background_executor()
                .timer(Self::GRAPH_REFRESH_DEBOUNCE)
                .await;

            let Some(entity) = weak_this.upgrade() else {
                return;
            };

            let Some(repo_root) = entity.update(cx, |this, cx| {
                    if epoch != this.graph_refresh_epoch {
                        return None;
                    }
                    if this.snapshot_loading {
                        return None;
                    }
                    let Some(repo_root) = this.repo_root.clone() else {
            ...
            let graph_result = cx
                .background_executor()
                .spawn(async move {
                    load_graph_snapshot_without_refresh(&repo_root, GraphSnapshotOptions::default())
                })
                .await;

            if let Some(entity) = weak_this.upgrade() {
                entity.update(cx, move |this, cx| {
                    ...
                    match graph_result {
                        Ok(graph_snapshot) => {
                            info!(
                                "git workspace background graph refresh ready: epoch={} elapsed_ms={} nodes={} edges={} has_more={}",
                                epoch,
                        ...
                        Err(err) => {
                            warn!(
                                "git workspace background graph refresh failed: epoch={} elapsed_ms={} err={err:#}",
                                epoch,
                                started_at.elapsed().as_millis()
                            );
                        }
