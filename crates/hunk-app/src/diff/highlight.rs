...
pub fn build_syntax_only_line_segments(file_path: Option<&str>, line: &str) -> Vec<StyledSegment> {
    if line.is_empty() {
        return Vec::new();
    }
...
pub fn build_plain_line_segments(file_path: Option<&str>, line: &str) -> Vec<StyledSegment> {
...
fn intra_line_change_map(
    ...
    for (ix, token) in line_tokens.iter().enumerate() {
        if common_flags.get(ix).copied().unwrap_or(false) {
            continue;
        }
