...
use crate::ai::AiPromptSkillReference;
    ...
    specs.iter().any(|spec| match spec {
        DynamicToolSpec::Namespace(namespace) if namespace.name == expected_namespace => {
            namespace.tools.iter().any(|tool| match tool {
                DynamicToolNamespaceTool::Function(function) => function.name == expected_tool,
            })
        }
        _ => false,
    })
...
fn transient_rollout_error_helper_matches_empty_rollout_server_errors_only() {
    ...
    assert!(!is_transient_rollout_load_error(
        &CodexIntegrationError::JsonRpcServerError {
            code: -32603,
            message: "failed to load rollout '/tmp/rollout.jsonl': permission denied".to_string(),
        }
    ));
    assert!(!is_transient_rollout_load_error(
        &CodexIntegrationError::JsonRpcServerError {
...
fn request_id_keys_are_type_stable() {
    assert_eq!(request_id_key(&RequestId::Integer(7)), "int:7".to_string());
    assert_eq!(
...
fn reconnect_backoff_grows_and_caps() {
    ...
    assert_eq!(
        reconnect_backoff(9),
        std::time::Duration::from_millis(64_000)
    );
}
